...
final class ThreadMessageResponseAttachment implements ResponseContract
{
    ...
    {
        $tools = is_array($attributes['tools']) ? array_map(
            fn(array $tool): ThreadMessageResponseAttachmentFileSearchTool|ThreadMessageResponseAttachmentCodeInterpreterTool => match ($tool['type']) {
    ...
    {
        return [
            'file_id' => $this->fileId ?? null,
            'tools' => is_array($this->tools) ? array_map(
